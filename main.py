...
from actions.whatsapp  import send_whatsapp_message, save_whatsapp_contact, whatsapp_broadcast
from actions.media     import play_media
from actions.weather   import get_weather_summary
from actions.screen_vision import analyze_screen
from actions.youtube_stats import get_youtube_channel_report
from actions.location import find_location
from actions.social import instagram_tracker, social_post_scheduler, social_ad_manager
...
from actions.cron_scheduler import cron_scheduler
...
class JarvisLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "save_memory":
            ...
            elif name == "whatsapp_broadcast":
                r = await loop.run_in_executor(
                    None,
                    lambda: whatsapp_broadcast(
                        args.get("recipients", ""),
                        args.get("message", ""),
                        bool(args.get("send_now", False)),
                    ),
                ...
                r = await loop.run_in_executor(
                    None,
                    lambda: find_location(
                        args.get("query", ""),
                        args.get("target", "device"),
                        bool(args.get("open_maps", False)),
                    ),
                )
                result = r or "Konum sorgusu tamamlandı."

            elif name == "instagram_tracker":
                r = await loop.run_in_executor(
                    None,
                    lambda: instagram_tracker(
                        args.get("username", ""),
                        args.get("action", "check"),
                    ),
                )
                result = r or "Instagram analizi tamamlandı."

            elif name == "social_post_scheduler":
                r = await loop.run_in_executor(
                    None,
                    lambda: social_post_scheduler(
                        args.get("platform", "instagram"),
                        args.get("content", ""),
                        args.get("scheduled_time", ""),
                        args.get("media_url", ""),
                    ),
                )
                result = r or "Paylaşım zamanlandı."

            elif name == "social_ad_manager":
                r = await loop.run_in_executor(
                    None,
                    lambda: social_ad_manager(
                        args.get("action", "create"),
                        args.get("campaign_name", ""),
                        float(args.get("budget", 0.0) or 0.0),
                        args.get("target_audience", ""),
                    ),
                )
                result = r or "Reklam kampanyası işlemi tamamlandı."

            elif name == "smart_home_control":
                r = await loop.run_in_executor(
                    None,
                    lambda: smart_home_control(
                        args.get("device_type", "all"),
                ...
                result = r or "Akıllı ev işlemi tamamlandı."

            elif name == "file_organizer":
                r = await loop.run_in_executor(
                    None,
                    lambda: file_organizer(
                        args.get("directory_path", "~/Downloads"),
                        bool(args.get("dry_run", False)),
                        args.get("group_by", "type"),
                    ),
                )
                result = r or "Dosya düzenleme tamamlandı."

            elif name == "mail_agent":
                r = await loop.run_in_executor(
                    None,
                    lambda: mail_agent(
                        args.get("action", "unread"),
                        args.get("query", ""),
                        args.get("recipient", ""),
                        args.get("subject", ""),
                        args.get("body", ""),
            ...
            elif name == "cron_scheduler":
                r = await loop.run_in_executor(
                    None,
                    lambda: cron_scheduler(
                ...
                result = r or "Zamanlanmış görev işlemi tamamlandı."

            elif name == "survival_guide":
                r = await loop.run_in_executor(
                    None,
                    lambda: survival_guide(
                        args.get("topic", "emergency_numbers"),
                        args.get("query", ""),
                    ),
                )
                result = r or "Acil durum rehberi hazırlandı."

            elif name == "companion_mode":
                r = await loop.run_in_executor(
                    None,
                    lambda: companion_mode(
                        args.get("mode", "friendly"),
                        bool(args.get("enabled", True)),
                    ),
                )
                result = r or "Yol arkadaşı modu güncellendi."
