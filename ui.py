...
class JarvisUI:
    def __init__(self):
        ...
        self._phone_bridge  = None      # lazy — phone_bridge.PhoneBridge
        self._phone_photo   = None      # QR ImageTk referansı (GC koruması)
        self._phone_url     = ""
        self._phone_token   = ""
        self._phone_busy    = False     # başlatılıyor/durduruluyor kilidi
...
"""
    ...
    def _build_phone_panel(self):
        ...
        self._phone_btn_canvas = tk.Canvas(
            self.root, width=200, height=40, bg=C_BG,
            highlightthickness=0, cursor="hand2")
        self._phone_btn_canvas.bind("<Button-1>", lambda e: self._toggle_phone_panel())
        self._draw_phone_button()

        # Panel (ayarlar gibi açılıp kapanan overlay)
        self._phone_panel = tk.Frame(
            self.root, bg="#041111", highlightbackground=C_MID, highlightthickness=1)
        self._phone_panel.place_forget()

        head = tk.Frame(self._phone_panel, bg="#041111")
        head.pack(fill="x", padx=12, pady=(12, 6))
        tk.Label(head, text="JARVIS TELEFON", fg=C_PRI, bg="#041111",
                 font=font_display(11)).pack(side="left")
        close = tk.Label(head, text="✕", fg=C_DIM, bg="#041111",
                         font=font_body_bold(13), cursor="hand2")
        close.pack(side="right")
        close.bind("<Button-1>", lambda e: self._toggle_phone_panel())

        # QR alanı — sabit piksel yükseklikli çerçeve (yerleşim sabit kalsın)
        qr_holder = tk.Frame(self._phone_panel, bg="#041111", width=250, height=250)
        qr_holder.pack(padx=12, pady=(4, 6))
        qr_holder.pack_propagate(False)
        self._phone_qr_label = tk.Label(
            qr_holder, bg="#041111", fg=C_MID, font=font_body(10),
            justify="center",
            text="Telefondan bağlanmak için\naşağıdaki BAŞLAT'a bas.")
        self._phone_qr_label.pack(expand=True)

        self._phone_status_label = tk.Label(
            self._phone_panel, text="Durum: kapalı", fg=C_MID, bg="#041111",
            font=font_body(9))
        self._phone_status_label.pack(pady=(0, 6))

        # Token satırı: kutu + kopyala
        trow = tk.Frame(self._phone_panel, bg="#041111")
        trow.pack(fill="x", padx=12, pady=(0, 8))
        self._phone_token_label = tk.Label(
            trow, text="", fg=C_TEXT, bg="#020a0a", font=font_body(9),
            anchor="w", wraplength=210, justify="left", padx=8, pady=6,
            highlightthickness=1, highlightbackground=C_DIM)
        self._phone_token_label.pack(side="left", fill="x", expand=True)
        self._phone_copy = tk.Label(
            trow, text="⧉", fg=C_PRI, bg="#062020", font=font_body_bold(14),
            cursor="hand2", padx=9, pady=4)
        self._phone_copy.bind("<Button-1>", lambda e: self._copy_phone_token())

        # Başlat / Durdur butonu
        self._phone_toggle_canvas = tk.Canvas(
            self._phone_panel, width=250, height=40, bg="#041111",
            highlightthickness=0, cursor="hand2")
        self._phone_toggle_canvas.bind("<Button-1>", lambda e: self._toggle_phone_server())
        self._phone_toggle_canvas.pack(pady=(0, 12))
        self._draw_phone_toggle()

    def _draw_phone_button(self):
        c = self._phone_btn_canvas
        bw, bh = int(c["width"]), int(c["height"])
        c.delete("all")
        running = bool(self._phone_bridge and self._phone_bridge.running)
        accent = C_GREEN if running else (C_BLUE if self._phone_open else C_MID)
        inner  = "#062020" if self._phone_open else "#021010"
        c.create_rectangle(0, 0, bw, bh, fill=inner, outline="")
        bl = 9
        for bx, by, sx, sy in [(0, 0, 1, 1), (bw, 0, -1, 1), (0, bh, 1, -1), (bw, bh, -1, -1)]:
            c.create_line(bx, by, bx + sx * bl, by, fill=accent, width=2)
            c.create_line(bx, by, bx, by + sy * bl, fill=accent, width=2)
        dot = "●" if running else "○"
        c.create_text(14, bh // 2, text=f"{dot}  JARVIS TELEFON",
                      fill=(C_GREEN if running else C_PRI),
    ...
    def _draw_phone_toggle(self):
        c = self._phone_toggle_canvas
        bw, bh = int(c["width"]), int(c["height"])
        c.delete("all")
        running = bool(self._phone_bridge and self._phone_bridge.running)
        if self._phone_busy:
            col, label = C_GOLD, "LÜTFEN BEKLE..."
        elif running:
            col, label = C_RED, "■  DURDUR"
        else:
            col, label = C_GREEN, "▶  BAŞLAT"
        c.create_rectangle(0, 0, bw, bh, fill="#021010", outline=col, width=1)
        c.create_text(bw // 2, bh // 2, text=label, fill=col, font=font_body_bold(12))

    def _toggle_phone_server(self):
        if self._phone_busy:
            return
        bridge = self._ensure_phone_bridge()
        if bridge.running:
            self._phone_busy = True
            self._draw_phone_toggle()
            self._set_phone_status("durduruluyor...")
            def _stop():
                bridge.stop()
                self.root.after(0, self._phone_stopped_ui)
            threading.Thread(target=_stop, daemon=True).start()
        else:
            self._phone_busy = True
            self._draw_phone_toggle()
            self._set_phone_status("başlatılıyor...")
            bridge.start(
                on_ready=lambda url, tok: self.root.after(0, self._on_phone_ready, url, tok),
                on_error=lambda msg: self.root.after(0, self._on_phone_error, msg),
                on_status=lambda s: self.root.after(0, self._set_phone_status, s),
            )

    def _ensure_phone_bridge(self):
        if self._phone_bridge is None:
            from phone_bridge import PhoneBridge
            self._phone_bridge = PhoneBridge()
        return self._phone_bridge

    def _set_phone_status(self, text: str):
        self._phone_status_label.configure(text=f"Durum: {text}")

    def _on_phone_ready(self, full_url: str, token: str):
        self._phone_busy = False
        self._phone_url = full_url
        self._phone_token = token
        self._set_phone_status("çalışıyor — QR'ı okut")
        self._phone_token_label.configure(text=token or "(token yok)")
        self._phone_copy.pack(side="left", padx=(6, 0))
        self._render_phone_qr(full_url)
        self._draw_phone_toggle()
        self._draw_phone_button()
        self.write_log("SYS: JARVIS Telefon açık — QR'ı okut.")

    def _on_phone_error(self, msg: str):
        self._phone_busy = False
        self._set_phone_status(f"hata: {msg}")
        self._draw_phone_toggle()
        self._draw_phone_button()
        self.write_log(f"SYS: JARVIS Telefon hatası — {msg}")

    def _phone_stopped_ui(self):
        self._phone_busy = False
        self._phone_url = ""
        self._phone_token = ""
        self._phone_photo = None
        self._phone_qr_label.configure(
            image="", text="Telefondan bağlanmak için\naşağıdaki butona bas.")
        self._phone_token_label.configure(text="")
        self._phone_copy.pack_forget()
        self._set_phone_status("kapalı")
        self._draw_phone_toggle()
        self._draw_phone_button()
    ...
    def _render_phone_qr(self, url: str):
        try:
            ...
            qr.add_data(url)
            qr.make(fit=True)
            img = qr.make_image(fill_color="#0b1a1a", back_color="#d8fff7").convert("RGB")
            img = img.resize((230, 230), Image.Resampling.NEAREST)
            self._phone_photo = ImageTk.PhotoImage(img)
            self._phone_qr_label.configure(image=self._phone_photo, text="")
        except Exception as exc:
            self._phone_qr_label.configure(
                image="", text=f"QR üretilemedi.\nAdres:\n{url}")
            print(f"[UI] QR hatası: {exc}")

    def _copy_phone_token(self):
        if not self._phone_token:
            return
        self.root.clipboard_clear()
        self.root.clipboard_append(self._phone_token)
        self._phone_copy.configure(text="✓")
        self.root.after(1200, lambda: self._phone_copy.configure(text="⧉"))
        self.write_log("SYS: Token panoya kopyalandı.")

    def _shutdown(self):
        if self._phone_bridge and self._phone_bridge.running:
            try:
                self._phone_bridge.stop()
            except Exception:
                pass
        self.sound.stop_all()
    ...
    def _place_layout_widgets(self):
        ...
        # JARVIS Telefon butonu (sol-alt) + paneli
        if hasattr(self, "_phone_btn_canvas"):
            phone_btn_y = self.H - FOOTER_H - 52 - 48
            self._phone_btn_canvas.place(x=14, y=phone_btn_y)
            if self._phone_open:
                pw, ph = 300, 470
                py = max(HDR_H + 10, phone_btn_y - ph - 8)
                self._phone_panel.place(x=14, y=py, width=pw, height=ph)
                self._phone_panel.lift()
            else:
                self._phone_panel.place_forget()
