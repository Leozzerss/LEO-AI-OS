...
def whatsapp_broadcast(recipients: list[str] | str, message: str, send_now: bool = False) -> str:
    """
    Birden fazla kisiye toplu WhatsApp mesaji gonderir veya taslak olusturur.
    recipients: Isim/numara listesi veya virgulle ayrilmis metin.
    """
    if isinstance(recipients, str):
        target_list = [r.strip() for r in re.split(r"[,;\n]", recipients) if r.strip()]
    else:
        target_list = [str(r).strip() for r in (recipients or []) if str(r).strip()]

    if not target_list:
        return "Toplu mesaj icin en az bir alici belirtilmeli."
    if not message or not message.strip():
        return "Gonderilecek mesaj metni bos olamaz."

    results = []
    for r in target_list:
        is_phone = bool(re.search(r"\d{7,}", r))
        if is_phone:
            res = send_whatsapp_message(message=message, phone_number=r, send_now=send_now)
        else:
            res = send_whatsapp_message(message=message, recipient_name=r, send_now=send_now)
        results.append(f"• {r}: {res}")

    return f"Toplam {len(target_list)} kisiye toplu mesaj islemi yurutuldu:\n" + "\n".join(results)
