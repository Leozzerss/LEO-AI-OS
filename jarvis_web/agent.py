...
def execute_tool(name: str, args: dict) -> str:
    ...
    try:
        ...
        if name == "whatsapp_broadcast":
            return whatsapp_broadcast(
                args.get("recipients", ""),
                args.get("message", ""),
                bool(args.get("send_now", False)),
            ) or "Toplu WhatsApp mesajı işlemi tamamlandı."
        ...
        if name == "find_location":
            ...
            ) or "Konum sorgusu tamamlandı."

        if name == "instagram_tracker":
            return instagram_tracker(
                args.get("username", ""),
                args.get("action", "check"),
            ) or "Instagram analizi tamamlandı."

        if name == "social_post_scheduler":
            return social_post_scheduler(
                args.get("platform", "instagram"),
                args.get("content", ""),
                args.get("scheduled_time", ""),
                args.get("media_url", ""),
            ) or "Paylaşım zamanlandı."

        if name == "social_ad_manager":
            return social_ad_manager(
                args.get("action", "create"),
                args.get("campaign_name", ""),
                float(args.get("budget", 0.0) or 0.0),
                args.get("target_audience", ""),
            ) or "Reklam kampanyası işlemi tamamlandı."
        ...
        if name == "smart_home_control":
            ...
            ) or "Akıllı ev işlemi tamamlandı."
